...
<?php foreach ( $option['row_options']['name']['choices'] as $row_name => $value ) : ?>
	...
	<div class="fw-table-cell-content <?php echo esc_attr($row_name) ?> <?php echo esc_attr($current_row_name === $row_name ? 'fw-active-content' : '') ?>">
		...
		<?php foreach ( $option['content_options'][ $row_name ] as $key => $options ) : ?>
			...
			<?php if ( $row_name == 'button-row' ): ?>
				<?php $options['button'] = esc_html__( 'Edit', 'leven' );
				if ( empty( $cell_value['button'] ) ) {
					$options['button'] = esc_html__( 'Add', 'leven' );
				} ?>
			<?php endif; ?>

...
class FW_Option_Type_Table extends FW_Option_Type
{
	...
	protected function _render( $id, $option, $data ) {
		...
		if ( ! $table_shortcode ) {
			trigger_error(
				__( 'table-builder option type must be inside the table shortcode', 'leven' ),
				E_USER_ERROR
			);
		}
	...
	protected function _get_defaults() {
		...
		return apply_filters( 'fw_option_type_table_defaults', array(
			'header_options'  => array(
				'table_purpose' => array(
					'type'    => 'select',
					'label'   => esc_html__( 'Table Styling', 'leven' ),
					'desc'    => esc_html__( 'Choose the table styling options', 'leven' ),
					'choices' => array(
						'pricing' => esc_html__( 'Use the table as a pricing table', 'leven' ),
						'tabular' => esc_html__( 'Use the table to display tabular data', 'leven' ),
					),
					'value'   => 'pricing',
					'attr'    => array(
			...
			'row_options'     => array(
				'name' => array(
					...
					'desc'    => false,
					'choices' => array(
						'default-row' => esc_html__( 'Default row', 'leven' ),
						'heading-row' => esc_html__( 'Heading row', 'leven' ),
						'pricing-row' => esc_html__( 'Pricing row', 'leven' ),
						'button-row'  => esc_html__( 'Button row', 'leven' ),
						'switch-row'  => esc_html__( 'Row switch', 'leven' )
					),
				)
			),
			'columns_options' => array(
				'name' => array(
					...
					'desc'    => false,
					'choices' => array(
						'default-col'   => esc_html__( 'Default column', 'leven' ),
						'desc-col'      => esc_html__( 'Description column', 'leven' ),
						'highlight-col' => esc_html__( 'Highlight column', 'leven' ),
						'center-col'    => esc_html__( 'Center text column', 'leven' )
					),
				)
			),
			'content_options' => array(
				...
				'pricing-row' => array(
					...
					'description' => array(
						...
						'value'        => '',
						'attr'         => array(
							'placeholder' => esc_html__( 'per month', 'leven' )
						),
						'wrapper_attr' => array(
							'class' => 'fw-col-sm-6'
						)
				...
				'button-row'  => array(
					'button' => ( $button = $table->get_button_shortcode() )
						? array(
							'type'          => 'popup',
							'popup-title'   => esc_html__( 'Button', 'leven' ),
							'button'        => esc_html__( 'Add', 'leven' ),

...
if ( fw_ext('portfolio') && function_exists( 'leven_filter_portfolio_extension' ) ) {
    ...
    if (empty($categories_selected_array)):
        ...
        $categories_selected_array_new = get_terms( $taxonomy, array(
            'hide_empty' => 0,
            'fields' => 'ids'
        ) );
        $categories_selected_string = implode(",", $categories_selected_array_new);
    else:
...
<?php if ( fw_ext('portfolio') && function_exists( 'leven_filter_portfolio_extension' ) ) { ?>                        
    <!-- Portfolio Content -->
    <div id="portfolio_content_<?php echo esc_attr($atts['id']); ?>" class="portfolio-content" data-categories="<?php echo esc_attr($categories_selected_string); ?>">
        ...
        <?php if ( ! empty( $categories ) ) : ?>
            <ul class="portfolio-filters">
                <li class="active">
                    <a class="filter btn btn-sm btn-link" data-group="category_all"><?php esc_html_e( 'All', 'leven-shortcodes' ); ?></a>
                </li>
                        ...
                        <figure class="item <?php echo esc_attr($portfolio_type); ?>" data-groups='["category_all"<?php fw_theme_portfolio_post_taxonomies_slug(get_the_ID()); ?>]'>
                            ...
                            <?php
                                if ( has_post_thumbnail() ) {
                                    ...
                                    ?>
                                        <img src="<?php echo get_template_directory_uri(); ?>/images/portfolio_no_image.jpg" class="portfolio-without-image" alt="<?php esc_attr_e('image', 'leven-shortcodes'); ?>" />
                                    <?php
                                }
                            ?>
